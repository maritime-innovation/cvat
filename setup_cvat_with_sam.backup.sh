...
log "Waiting CVAT API to be ready..."
for i in {1..60}; do
  if curl -fsS "http://${CVAT_HOST}:8080/api/server/about" >/dev/null; then
    echo "CVAT API is up."
    break
  fi
  sleep 2
...
docker exec -i cvat_server bash -lc "curl -fsS http://nuclio:8070/api/functions >/dev/null && echo OK || (echo NG; exit 1)"
